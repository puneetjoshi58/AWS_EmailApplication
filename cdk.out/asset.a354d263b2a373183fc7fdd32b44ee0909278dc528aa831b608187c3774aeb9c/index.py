    ...
    try:
        ...
        if 'Item' in response_dynamo:
            ...
            try:
                ...
                logger.exception('TIP: Confiration email has been sent !! \n Confirm the pending email verification and try again')
                return buildResponse(200,  {'Message':'Confiration email has been sent to : %s Email Address !! Confirm the pending email verification and try again' % Email_id})
            
            send_email(Email_id)
            
            return buildResponse(200,  {'Message':'Notification has been sent to : %s Email Address' % Email_id})
